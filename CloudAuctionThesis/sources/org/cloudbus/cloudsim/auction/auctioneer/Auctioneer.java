...
// norm comparator is used here
		...
		for (BidCalculation brokerBidCalc : brokerBidCalcs) {
			...
			if (brokerVMs != null) { // If broker has request
				for (BidCalculation datacenterBidCalc : datacenterBidCalcs) {
					 ...
					 */
					if (brokerBidCalc.getAverageBid() >= datacenterBidCalc.getAverageBid()) {   //comparison of bid
    ...
    private void algorithmEngine(BidCalculation brokerBidCalc, List<Vm> requestedVMs,
		...
		for (Vm requestedVM : requestedVMs) {
			for (Vm offeredVM : offeredVMs) {
				 ...
				 */
				//If both the users and providers request and provide vms then
				 ...
				 */
				// if statemeent to fulfil users request
				// Check that the proivder  can fullfil the user request ornot
				// If full fill the request then allocate services
	...
	private void assignVMs(BidCalculation brokerBidCalc, Vm requestedVM, 
		 ...
		 * Final trade price by me
		 */
		double price=(requestedVM.getQ1()+requestedVM.getQ2()+requestedVM.getQ3()+requestedVM.getQ4())/4; // Summation of QoS/2*No
		double newFinalTradePrce=payPrice+price;
		//in case of penality divid the newfinal price by the mean of Quality of services.
		//double penaltyprice= newFinalTradePrce/(requestedVM.getQ1()+requestedVM.getQ2()+requestedVM.getQ3()+requestedVM.getQ4())/4;
		brokerBidCalc.assignAllocation(requestedVM, assignAmount, newFinalTradePrce);//newFinalTradePrce);
		/**
		 ...
		 */
		datacenterBidCalc.assignAllocation(offeredVM, assignAmount, newFinalTradePrce);
		
		this.allocations.addAllocation(requestedVM, datacenterBidCalc, offeredVM, assignAmount, newFinalTradePrce);
 	}
...
// Average price
	...
	private void printAllocations() {
		Log.printLine();
		Log.printLine("#######Auction module winner determination #######");
					...
					for (Vm vm : allocs.keySet()) {							
						...
						if (datacenterIDs != null) {							
							for (Integer datacenterID : datacenterIDs.keySet()) {
								...
								// jis jis ko allocation hui hai os ko random number from 1 to 5 assign kary ga
								
								Random randomValues= new Random();
								AuctionExample5.tableOfRatings[bidCalc.getBidderID()-10][datacenterID-3]=new ProvidersRattings(randomValues.nextInt(5)+1, // we did get ranking here because 
...
*/		
		...
		Log.printLine("#######Auction Module publication of Allocation of winners#######");
		Log.printLine();
		
	}
	// Bid density publications of users and providers
    ...
    private void printBidCalcs(String name, List<BidCalculation> brokerBidCalcs) {
        ...
        for (BidCalculation calc : brokerBidCalcs) {
            System.out.println(calc.toString() + ", Bid Density=" + calc.getNorm());
        }
