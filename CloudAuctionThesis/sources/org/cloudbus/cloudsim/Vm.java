...
public class Vm {
	...
	/**Serial number for datacenter FRVd*/
	...
	/**check  if the VM is of type data center or not*/
	private boolean isOfTypeDataCenter;
	
	
	public boolean isOfTypeDataCenter() {
		return isOfTypeDataCenter;
	}

	public void setOfTypeDataCenter(boolean isOfTypeDataCenter) {
		this.isOfTypeDataCenter = isOfTypeDataCenter;
	}
	 ...
	 * @post $none
	 */
	public Vm(int serialNum, boolean isOfTypeDatacenter,double q1, double q2, double q3, double q4, int id, int userId, double mips, int pesNumber, int ram, long bw, long size, String vmm, CloudletScheduler cloudletScheduler) 
	    {
		setSerNumber(serialNum);
		setOfTypeDataCenter(isOfTypeDatacenter);
		setQ1(q1);
		setQ2(q2);
		setQ3(q3);
		setQ4(q4);
